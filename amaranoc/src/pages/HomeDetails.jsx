...
export default function HomeDetails() {
  ...
  if (!home) {
    return (
      <div className="flex h-screen w-screen flex-col items-center justify-center gap-4 bg-gray-50 px-4 text-center">
        ...
        <button
          onClick={() => navigate(-1)}
          className="rounded-full bg-[#f97316] px-5 py-2 text-white font-medium hover:bg-[#ea580c] transition-colors cursor-pointer"
        >
          Հետ գնալ
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-12 font-sans text-[#334155]">
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between bg-white rounded-2xl border border-slate-100 p-4 shadow-sm gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 lg:gap-6 w-full lg:w-auto justify-between lg:justify-end">
            <div className="flex gap-6 flex-wrap">
              ...
              {home.isSleep && (
                <div className="flex flex-col">
                  ...
                  <span className="text-base font-extrabold text-[#f97316]">
                    {home.sleepPrice?.toLocaleString()} ֏
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-1.5 bg-slate-50 p-1 rounded-xl border border-slate-200 self-end sm:self-auto">
              <button className="h-7 w-7 rounded-lg bg-slate-900 text-white text-xs font-bold shadow-sm cursor-pointer">
                ֏
              </button>
              <button className="h-7 w-7 rounded-lg text-slate-500 text-xs font-bold hover:bg-slate-200 cursor-pointer transition-colors">
                $
              </button>
              <button className="h-7 w-7 rounded-lg text-slate-500 text-xs font-bold hover:bg-slate-200 cursor-pointer transition-colors">
                €
      ...
      {/* --- RESPONSIVE MEDIA GRID SECTION --- */}
      <div className="mx-auto max-w-[1300px] px-4 pt-4">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-2 sm:gap-3">
          {/* Main Hero Image */}
          <div
            onClick={() => openSlider(0)}
            className="md:col-span-6 lg:col-span-6 relative aspect-[4/3] md:aspect-auto md:h-[380px] lg:h-[440px] overflow-hidden rounded-2xl shadow-sm group cursor-pointer"
          >
            <img
            ...
            <button
              ...
              className={`absolute right-3 top-3 z-10 border-none bg-none text-base sm:text-lg cursor-pointer drop-shadow-[0_1px_3px_rgba(0,0,0,0.5)] transition-colors ${
                isLiked
                  ? "text-[#ef4444]"
                  : "text-white/70 hover:text-[#ef4444]"
              }`}
            >
            ...
            <button
              ...
              className="md:hidden absolute bottom-3 right-3 flex items-center gap-1.5 bg-slate-900/80 backdrop-blur-md text-white text-xs font-medium px-3 py-1.5 rounded-lg shadow-md border border-white/20"
            >
              <FontAwesomeIcon icon={faImages} />
              <span>1/{propertyImages.length}</span>
            </button>
          </div>

          {/* Secondary 2x2 Image Grid */}
          <div className="hidden md:grid md:col-span-6 lg:col-span-6 grid-cols-2 gap-2 sm:gap-3 md:h-[380px] lg:h-[440px]">
            <div
              onClick={() => openSlider(1)}
              className="overflow-hidden rounded-2xl shadow-sm h-full relative group cursor-pointer"
            >
              <img
            ...
            <div
              ...
              className="relative overflow-hidden rounded-2xl shadow-sm h-full group cursor-pointer"
            >
              <img
              ...
              <button className="absolute bottom-3 right-3 bg-white/95 backdrop-blur-md hover:bg-white text-slate-800 text-[11px] font-bold px-3 py-1.5 rounded-xl shadow-md border border-slate-100 transition-all cursor-pointer flex items-center gap-1.5">
                <FontAwesomeIcon icon={faImages} className="text-slate-500" />
                <span>Տեսնել բոլորը ({propertyImages.length})</span>
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="mx-auto mt-5 grid max-w-[1300px] grid-cols-1 gap-5 px-4 lg:grid-cols-2">
        <div className="bg-white rounded-2xl border border-slate-100 p-4 sm:p-5 shadow-sm h-fit">
          ...
          <div className="flex flex-col gap-3 text-sm">
            ...
            <div className="flex justify-between items-center border-b border-slate-50 pb-1.5">
              ...
              <span className="font-semibold text-slate-800">
                {home.peopleCaunt || home.peopleCount}
              </span>
            </div>
            <div className="flex justify-between items-center border-b border-slate-50 pb-1.5">
              <span className="text-slate-400">🛏️ Քանակ գիշերակացով</span>
              <span className="font-semibold text-slate-800">
                {home.peopleSleepCaunt || 0}
              </span>
            </div>
            <div className="flex justify-between items-center border-b border-slate-50 pb-1.5">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-4 sm:p-5 shadow-sm h-fit">
          ...
          <div className="grid grid-cols-7 text-center text-[11px] sm:text-xs font-bold text-slate-400 mb-3">
            ...
            <span>ուրբ</span>
            <span>շբթ</span>
            <span className="text-orange-500">կիր</span>
          </div>
          ...
          <div className="grid grid-cols-7 gap-1 text-center text-xs sm:text-sm font-semibold text-slate-700">
            {prevMonthDays.map((day, idx) => (
              <span key={`prev-${idx}`} className="text-slate-300 p-2 sm:p-2.5">
                {day}
              </span>
            ))}
            ...
            {nextMonthDays.map((day, idx) => (
              <span key={`next-${idx}`} className="text-slate-300 p-2 sm:p-2.5">
                {day}
              </span>
            ))}
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-4 sm:p-6 shadow-sm flex flex-col gap-6">
          <div>
            ...
            <p className="text-sm leading-relaxed text-slate-600 font-medium">
              {home.addres}ում օրավարձով է տրվում ընդարձակ և գեղեցիկ տնակ՝
              ստեղծված ընտանեկան, ընկերական և կոլեկտիվ միջոցառումների համար։
              Տնակը նախատեսված է {home.peopleCaunt || home.peopleCount} անձի
              համար։
            </p>
          </div>
            ...
            {home.advantages && (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5">
                {home.advantages.map((adv, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-2 text-sm text-slate-700 bg-slate-50 p-2.5 rounded-xl border border-slate-100/50"
                  >
                    <FontAwesomeIcon
                      icon={faCheckCircle}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t border-slate-100 text-sm">
            <div>
              ...
              <ul className="list-disc pl-5 space-y-1 text-slate-600">
                <li>
                  Մինչև 20 անձի համար՝{" "}
                  <span className="font-semibold text-slate-800">
                    {home.price?.toLocaleString()} դրամ
                  </span>
                </li>
                {home.sleepPrice && (
                  <li>
                    Գիշերակացով արժեքը՝{" "}
                    <span className="font-semibold text-slate-800">
                      {home.sleepPrice?.toLocaleString()} դրամ
                    </span>
                  </li>
                )}
              </ul>
            ...
            <div>
              ...
              <ul className="list-disc pl-5 space-y-1 text-slate-600">
                <li>
                  Մուտք՝{" "}
                  <span className="font-semibold text-slate-800">14:00</span>
                </li>
                <li>
                  Ելք՝{" "}
                  <span className="font-semibold text-slate-800">12:00</span>
                </li>
              </ul>
            </div>
          </div>
          ...
          <div className="text-xs text-slate-400 font-medium pt-2 border-t border-slate-50">
            Ամրագրման կամ այլ մանրամասների համար գրեք կամ զանգահարեք նշված
            հեռախոսահամարով:
          </div>
        </div>
      </div>
